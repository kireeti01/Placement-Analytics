...
exports.requestRegistration = async (req, res) => {
  try {
    ...
    const { college_name, college_code, admin_name, admin_email } = requestData;
    if (!college_name || !college_code || !admin_name || !admin_email) {
      return res.status(400).json({ error: 'Missing required fields: college_name, college_code, admin_name, admin_email' });
    }
    ...
    // Return validation errors from Sequelize as 400
...
exports.resetAdminPassword = async (req, res) => {
  try {
    ...
    if (!trimmedPassword || trimmedPassword.length < 6) {
      console.log('No valid new password provided');
